...
<div id="north-panel" class="north-panel" data-options="region:'north',border:false" style="height:64px;">
	<div class="north-top">
		<span class="logo">银象水泵管理系统</span>
		<span class="top-ico logout" onclick="logout()">退出登录</span>
...
<div data-options="region:'west'" style="width:220px;background:#3a8ef3;oveflow-x:hidden;">
...
<div id="center-panel" class="center-panel" data-options="region:'center'">
	<div id="eTab" class="easyui-tabs" data-options="border:false" style="width:100%;">
		<div title="主页" id="myIndex">
			<iframe id="indexF" width='100%' style="border:0;background:#fff url(${img}/bg.png) no-repeat center center;background-size:cover;"></iframe>
		</div> 
	</div>
</div>
<div data-options="region:'south'" style="height:36px;display:none;"><div id="footer">版权所有：优网助帮</div></div>
<script>
$(function(){
	resize();
	loadMenuTree();
...
function showRightMenu(){
	if($("#tabRightMenu").length <=0){
		...
		closeMenu.attr("name","closeMenu").attr("iconCls","icon-clear").html("close");
		closeOther.attr("name","closeOther").attr("iconCls","icon-remove").html("close other");	
		closeAll.attr("name","closeAll").attr("iconCls","icon-no").html("close all");	
